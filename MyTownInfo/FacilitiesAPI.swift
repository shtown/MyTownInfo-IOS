...
struct FacilitiesAPI  {
    ...
    fileprivate static let baseURLString = "https://gis.southamptontownny.gov/mytowninfo/getmytowninfojson.ashx"
    ...
    fileprivate static func facilityFromJSONObject(_ json: [String: AnyObject]) -> Facility?  {
        ...
        guard let
            ...
            let name = json["Website_Name"] as? String else {
                return nil
        }
    
        return Facility(ID:id, Title: title, Desc:desc, URL:url, Name:name)
        /*
        guard let
        ...
        return Facility(ID:id, Description:desc, URL:url, Website_Name:name, WebLink: url, Fee: fee, Lat: lat, Lon: lon, Category: cat, Hamlet: hamlet, Email: email, DistFromCenter: 0.0)
         */
